...
// FIX 1: Lấy PORT từ môi trường (mặc định 9002)
...
async function bootstrapData() {
  ...
  try {
    ...
    if (!Array.isArray(tickers)) return console.log("LOG: [PP3] API Binance lỗi/block IP");
...
// FIX 2: Bẫy lỗi try/catch cho WebSocket
async function initWS() {
    try {
      const res = await fetch('https://fapi.binance.com/fapi/v1/ticker/price');
      const tickers = await res.json();
      if (!Array.isArray(tickers)) return setTimeout(initWS, 3000);
...
// FIX 3: Khởi chạy Express trước
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 LUFFY ENGINE READY: http://0.0.0.0:${PORT}/gui`);
  bootstrapData();
  initWS();
  fallbackAPI();
});
